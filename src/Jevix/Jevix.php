...
class Jevix
{
    ...
    protected function tag(&$tag, &$params, &$content, &$short)
    {
        ...
        } elseif (array_key_exists($tag, $this->tagsRules) && array_key_exists(
                self::TR_TAG_CALLBACK,
                $this->tagsRules[$tag]
            )
        ) {
    ...
    protected function makeTag($tag, $params, $content, $short, $parentTag = null)
    {
            ...
            if (is_string($paramAllowedValues)) {
                switch ($paramAllowedValues) {
                    ...
                    case '#link':
                        ...
                        if (!preg_match('/^(http|https|ftp):\/\//ui', $value) && !preg_match(
                                '/^(\/|\#)/ui',
                                $value
                            )
                        ) {
                    ...
                    case '#image':
                        ...
                        // HTTP в начале если нет
                        if (!preg_match('/^(http|https):\/\//ui', $value) && !preg_match(
                                '/^\//ui',
                                $value
                            )
                        ) {
        ...
        $requiredParams = isset($tagRules[self::TR_PARAM_REQUIRED]) ? array_keys(
            $tagRules[self::TR_PARAM_REQUIRED]
        ) : array();
        if ($requiredParams) {
        ...
        if (!empty($tagRules[self::TR_PARAM_AUTO_ADD])) {
            foreach ($tagRules[self::TR_PARAM_AUTO_ADD] as $name => $aValue) {
                ...
                if (!array_key_exists(
                        $name,
                        $resParams
                    ) or ($aValue['rewrite'] and $resParams[$name] != $aValue['value'])
                ) {
        ...
        while (($this->curCh != '<') && $this->curChClass) {
            ...
            if (!$spCount && $this->curCh == '&' && $this->htmlEntity($entity)) {
                ...
                if ($this->curParentTag
                    ...
                    and (is_null(
                            $this->openedTag
                        ) or isset($this->tagsRules[$this->openedTag][self::TR_TAG_NO_AUTO_BR]))
                ) {
                    // пропускаем <br/>
                } else {
                    $br = $this->br . $this->nl;
